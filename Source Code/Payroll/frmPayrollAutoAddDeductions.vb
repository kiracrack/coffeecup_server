...
Public Class frmPayrollAutoAddDeductions
    ...
    End Sub
    Public Sub LoadData()
        LoadXgrid("select id,case when autoaddtype = 0 then (select description from tblemployeetype where code = tblpayrollautoadddeductions.subjectto) else (select description from tblpayrollbasicrate where code = tblpayrollautoadddeductions.subjectto) end as 'Subject to', " _
                            + " (select description from tblpayrolldedaddtype where code =  tblpayrollautoadddeductions.addcode) as 'Deduction Type', (select description from tblpayrollschedule where code=tblpayrollautoadddeductions.schedule) as 'Schedule' ,Amount, case when active =1 then 'Active' else 'Inactive' end as 'Status' from tblpayrollautoadddeductions where autoaddtype='" & RadioGroup1.SelectedIndex & "'", "tblpayrollautoadddeductions", Em, Gridview1, Me)
    ...
    Public Sub LoadDetails(ByVal id As String)
        ...
        com.CommandText = "select *,(select description from tblpayrolldedaddtype where code =  tblpayrollautoadddeductions.addcode) as 'additional', " _
                                + " (select description from tblpayrollschedule where code=tblpayrollautoadddeductions.schedule) as 'sched', " _
                                + " case when autoaddtype = 0 then (select description from tblemployeetype where code = tblpayrollautoadddeductions.subjectto) else (select description from tblpayrollbasicrate where code = tblpayrollautoadddeductions.subjectto) end  as 'subjecttodesc' from tblpayrollautoadddeductions where id='" & id & "'" : rst = com.ExecuteReader
        While rst.Read
    ...
    Private Sub cmdAttendanceSettings_Click(sender As Object, e As EventArgs) Handles cmdAttendanceSettings.Click
        ...
        If mode.Text = "edit" Then
            com.CommandText = "update tblpayrollautoadddeductions set autoaddtype = '" & RadioGroup1.SelectedIndex.ToString & "', " _
        ...
        Else
            com.CommandText = "insert into tblpayrollautoadddeductions set autoaddtype = '" & RadioGroup1.SelectedIndex.ToString & "', " _
                                    + " subjectto='" & employeetypecode.Text & "', " _
                                    + " addcode='" & dedaddcode.Text & "', " _
                                    + " schedule = '" & schedulecode.Text & "', " _
    ...
    Private Sub ActiveInactiveToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles cmdActiveInactive.Click
        If CheckSelectedRow("Subject to", Gridview1) = True Then
            If XtraMessageBox.Show("Are you sure you want to continue? ", compname, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
                If Gridview1.GetFocusedRowCellValue("Status").ToString = "Active" Then
                    com.CommandText = "update tblpayrollautoadddeductions set active=0 where id='" & Gridview1.GetFocusedRowCellValue("id").ToString & "'" : com.ExecuteNonQuery()
                Else
                    com.CommandText = "update tblpayrollautoadddeductions set active=1 where id='" & Gridview1.GetFocusedRowCellValue("id").ToString & "'" : com.ExecuteNonQuery()
                End If
                LoadData()
            End If
        End If
