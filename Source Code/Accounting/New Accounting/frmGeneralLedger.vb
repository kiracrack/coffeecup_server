...
Public Class frmGeneralLedger
    ...
    Public Sub LoadStructure()
        ...
        com.CommandText = "CALL sp_acct_generalledger('" & companyid.Text & "', '" & CDate(txtReportDate.EditValue).ToString("yyyy") & "', '" & ConvertDate(txtReportDate.EditValue) & "')" : com.ExecuteNonQuery()
        LoadXgrid("SELECT a.itemcode, a.itemname," & GlobalGLitemname & " as 'Particulars', if(debit=1,totaldebit-totalcredit,totalcredit-totaldebit) as 'Total', if(a.glgroup=1,1,0) as glgroup, if(a.gl=1,1,0) as gl, if(a.sl=1,1,0) as sl, a.level, a.parent  FROM `tmpGeneralLedger` as a order by a.groupcode,a.itemcode;", "tmpGeneralLedger", Em, GridView1, Me)
    ...
    Private Sub ViewLedgerToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ViewLedgerToolStripMenuItem.Click
        frmAccountLedger.companyid.Text = companyid.Text
        frmAccountLedger.txtYear.Text = CDate(txtReportDate.EditValue).ToString("yyyy")
        frmAccountLedger.accountno.Text = GridView1.GetFocusedRowCellValue("itemcode").ToString
        frmAccountLedger.txtAccountTitle.Text = GridView1.GetFocusedRowCellValue("itemname").ToString
        If frmAccountLedger.Visible = True Then
            frmAccountLedger.Focus()
        Else
            frmAccountLedger.Show(Me)
        End If
        frmAccountLedger.FilterAccountLedger()
    End Sub
End Class