...
Public Class frmProductDirect3
    ...
    Private Sub SimpleButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSaveParticular.Click
        ...
        com.CommandText = "Update tblglobalproducts set itemname='" & rchar(txtitem.Text) & "', barcode='" & txtBarcode.Text & "', partnumber='" & rchar(txtPartNumber.Text) & "', unit='" & txtUnit.Text & "', purchasedprice='" & Val(CC(txtPurchasePrice.Text)) & "',  sellingprice='" & Val(CC(txtSellingPrice.Text)) & "', approved=" & If(LCase(globaluser) = "root", True, False) & " where productid='" & productid.Text & "' " : com.ExecuteNonQuery()
        ...
        If frmProductManagement.Visible = True Then
            frmProductManagement.UpdateProduct(txtitem.Text, txtUnit.Text, txtPurchasePrice.Text, txtSellingPrice.Text, txtPartNumber.Text)
        End If
        Me.Close()
    End Sub
