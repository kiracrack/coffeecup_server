...
Public Class frmInvoinceChangeProduct
    ...
    Private Sub frmInvoinceChangeProduct_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub filter()
        LoadXgrid("SELECT productid,tblprocategory.catid, if(partnumber='' or partnumber is null,itemname,concat(itemname,'-',tblglobalproducts.description)) as 'Select Product' , " _
                  + " Unit from tblglobalproducts inner join tblprocategory on  tblprocategory.catid = tblglobalproducts.catid where enablesell=1 and " & SearchProductName("itemname", txtfilter.Text) & " and deleted=0 order by itemname asc", "tblglobalproducts", Em, inventoryGrid, Me)
    ...
    Public Sub ShowStockoutQuan()
        If CheckSelectedRow("productid", inventoryGrid) = True Then
            ...
            If XtraMessageBox.Show("Are you sure you want to continue?", compname, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
                com.CommandText = "UPDATE tblsalesclientchargesitem set productid='" & inventoryGrid.GetFocusedRowCellValue("productid").ToString() & "', productname='" & rchar(inventoryGrid.GetFocusedRowCellValue("Select Product").ToString()) & "',catid='" & inventoryGrid.GetFocusedRowCellValue("catid").ToString() & "' where id='" & trnid_invoice.Text & "'" : com.ExecuteNonQuery()
                com.CommandText = "UPDATE tblsalestransaction set productid='" & inventoryGrid.GetFocusedRowCellValue("productid").ToString() & "', productname='" & rchar(inventoryGrid.GetFocusedRowCellValue("Select Product").ToString()) & "',catid='" & inventoryGrid.GetFocusedRowCellValue("catid").ToString() & "' where id='" & trnid_batch.Text & "'" : com.ExecuteNonQuery()
                frmClientChargeInvoice.LoadInvoiceInfo()
                Me.Close()
