...
Module AutoUpdater
    ...
    Public Function AvailableNewUpdate() As Boolean
        com.CommandText = "select *, date_format(str_to_date(version, '%Y.%m.%d'), '%Y-%m-%d') as dt from tblclientsystemupdate where date_format(str_to_date(version, '%Y.%m.%d'), '%Y-%m-%d') > '" & ConvertDate(dversion) & "' and isserver=1 order by date_format(str_to_date(version, '%Y.%m.%d'), '%Y-%m-%d') asc limit 1" : rst = com.ExecuteReader
        If rst.Read Then
